...
class MMDB:
    ...
    def __init__(self, filename):
        with open(filename, 'rb') as input:
            ...
            try:
                print(self.data.rindex)
                pos = 0
                pos += len(MMDB.METADATA_BEGIN_MARKER)
            except:
                raise Exception(
                    "Could not find metadata marker, invalid database.")

            metadata_decoder = Decoder(self.data, pos)
            self.metadata = metadata_decoder.decode(pos)[0]
            self.metadata = Metadata(**self.metadata)
            self.decoder = Decoder(
                self.data, self.metadata.search_tree_size + MMDB.DATA_SECTION_SEPARATOR_SIZE)
    ...
    def _read_node(self, node_number, index):
        ...
        if record_size == 24:
            ...
            (middle,) = struct.unpack(
                b'!B', self.data[base_offset + 3:base_offset + 4])
            if index:
                middle &= 0x0F
            else:
